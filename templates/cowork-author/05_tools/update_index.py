...
- Both modes check that the schema's directories exist. The manifest records
  ...
  exit 1.
- Movement inside a frozen zone (01_basis/, 02_exchange/, 03_revisions/) is
  ...
  a session to resolve. The two LOG.md files are exempt when they CHANGE —
  appending to a log is the ordinary case (R4, R11) — and not when they go
  MISSING.
- CHANGED under 04_working/drafts/ is reported in its own section. The live
  draft is mutable and changes every session, but a change that shows up at
...
FROZEN_DIRS = ("01_basis/", "02_exchange/", "03_revisions/")

# The logs live inside frozen zones but are the record OF those zones, not
# documents in them: every ingestion, issue and freeze appends a row (R4, R11).
# Their growth is routine and must not raise the frozen-zone alarm, or the
# alarm fires on the ordinary case and stops being read. Losing one is a
# different matter, so the exemption covers CHANGED only, never MISSING.
FROZEN_EXEMPT = ("02_exchange/LOG.md", "03_revisions/LOG.md")

# The live drafts (R6). Mutable by definition — but a change visible at
# session start happened since the last index, which means outside a session:
# the user's hands were on the draft. Reported by itself so the session reads
...
REQUIRED_DIRS = (
    "00_AI_context", "00_AI_context/datasets", "00_AI_context/documents",
    "01_basis", "01_basis/reference", "01_basis/examples", "01_basis/templates",
...
def diff(root, args):
    ...
    # A live draft that changed since the last index changed outside a
    # session — the user edited it by hand. Read before writing (R6), and say
    # what changed: draft_diff.py against the last frozen revision.
    draft_changed = [p for p in changed if draft(p)]
    if draft_changed:
        print("DRAFT EDITED since last index — read it before editing it, and "
              "diff it against the last frozen revision (R6, draft_diff.py):")
        for p in draft_changed:
            print(f"  CHANGED  {p}")
        print()
    ...
    if not dirty:
        ...
        print(f"\n{len(added)} new, {len(changed)} changed, {len(missing)} "
              ...
              f"frozen zone"
              + (f", {len(draft_changed)} in a live draft" if draft_changed
                 else "")
