...
Keep them in `04_working/` and list them here so the exclusion is written down
somewhere both you and a session will see it.

The deliverable's own internal revisions are **not** this: they are frozen in
`03_revisions/` and logged in `03_revisions/LOG.md` (R11), and the `issued`
row there names which internal revision went out under which external label.
This log carries only the event of its leaving.
