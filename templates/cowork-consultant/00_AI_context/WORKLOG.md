...
- `04_tools/update_index.py` generates INDEX.md and MANIFEST.json, and counts
  whatever is waiting in `_inbox/` on every scan; initial baseline generated
  today.
...
- `_inbox/` is the drop zone: files land there undecided, and a session files
  them by the ingest procedure, asking where it cannot tell.
